...
*   一个显然的结论是，若能找出一种不包含第二个格子的子矩形使得其权值和大于等于 $2K$，这个子矩阵一定有一部分的权值和小于等于 $2K$，因为里面每个元素都不超过 $K$。
...
## POJ2373 [USACO2004 Dec] Dividing the Path

*   $f[i]$ 表示恰好灌溉前 $i$ 个区域至少需要的喷灌器数。
*   $f[i] = \min \\{ f[j] \\} + 1, l \le j \le r$
*   $l, r$ 可以由 $A, B$ 和牛的限制 $\mathcal O(1)$ 算出。
*   用单调队列优化 DP。
...
*   将有 $a, b$ 两种属性的装备看作点 $a, b$ 之间点无向边。
*   对于一个连通块，若不含环，则必定可以满足其中任意 $size - 1$ 个点。
*   对于一个连通块，若含环，则必定可以满足全部 $size$ 个点。
*   用并查集维护，记录每个连通块是否含环。
*   考虑合并：
    *   若 $x \neq y$，将数字小的连通块合并到数字大的连通块中，并将数字小的连通块打上标记；
    *   若 $x = y$，则存在环，将根结点打上标记。
*   考虑性质：
    *   所有结点均打过标记；
    *   除根结点（数字最大的结点）外所有结点均打过标记。
*   遍历统计答案即可。
